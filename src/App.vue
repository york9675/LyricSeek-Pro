...
const darkModeHandle = async (themeMode) => {
  if (themeMode === 'dark' || themeMode === 'light') {
    try {
      ...
      console.warn('Cannot update native window theme:', error)
    }
  } else {
    try {
      await appWindow.setTheme(null)
    } catch (error) {
      console.warn('Cannot reset native window theme:', error)
    }
    ...
    try {
      ...
      console.warn('Cannot read window theme, using current DOM theme:', error)
    }
  }

...
<div class="flex flex-col justify-center items-center w-full h-full p-4">
   <div class="m-4 text-hoa-1500 text-sm">{{ t('selectStrategy.title') }}</div>
   <div class="flex mb-4 gap-4 text-gray-200">
      <button @click="chooseCreateLRC" class="text-left flex-1 border-4 rounded p-3 transition" :class="{ 'border-hoa-1400 bg-hoa-800/70': isCreateLRC, 'border-transparent bg-hoa-800/30': !isCreateLRC }">
         <div class="font-bold text-hoa-1400 mb-3">{{ t('selectStrategy.createLrcTitle') }}</div>
         <div class="text-sm text-hoa-1300">{{ t('selectStrategy.createLrcDescription') }}</div>
      </button>
      ...
      <button @click="chooseEmbed" class="text-left flex-1 border-4 rounded p-3 transition" :class="{ 'border-hoa-1400 bg-hoa-800/70': isEmbed, 'border-transparent bg-hoa-800/30': !isEmbed }">
         <div class="font-bold text-hoa-1400 mb-3">{{ t('selectStrategy.embedTitle') }}</div>
         <div class="text-sm text-hoa-1300">{{ t('selectStrategy.embedDescription') }}</div>
      </button>
   </div>
   ...
   <div class="">
    <div class="flex items-center">
      ...
      <label for="skip-tracks" class="ml-2 text-sm font-medium text-hoa-1500">{{ t('selectStrategy.skipTracks') }}</label>
    </div>
   </div>

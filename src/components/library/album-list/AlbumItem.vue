<template>
  ...
  >
    ...
    <div class="flex-none w-16 h-16 flex items-center justify-center p-1 flex-shrink-0">
      <div v-if="coverImageUrl" class="w-14 h-14 rounded overflow-hidden">
        <img :src="coverImageUrl" :alt="album?.name" class="w-full h-full object-cover cursor-pointer" @click="$emit('openAlbum', album)">
      </div>
      <div v-else class="w-14 h-14 rounded bg-brave-90 dark:bg-brave-20 flex items-center justify-center cursor-pointer" @click="$emit('openAlbum', album)">
...
const loadCoverImage = async () => {
  if (!album.value?.image_path) {
    coverImageUrl.value = null
    return
  }

  try {
    const imageData = await invoke('get_cover_image', { imagePath: album.value.image_path })

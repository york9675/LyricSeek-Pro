    ...
    <div class="grow overflow-hidden flex flex-col gap-6">
      <form @submit.prevent="doSearchLyrics" class="flex flex-col flex-none gap-4">
        <div class="grid grid-cols-2 gap-2">
          ...
          <div>
            <label for="provider" class="group-label mb-1">Provider</label>
            <select id="provider" v-model="selectedProvider" class="input w-full py-1.5 px-2" :disabled="loading">
          ...
          <div v-if="selectedProvider === 'lrclib'">
            <label for="lrclibTag" class="group-label mb-1">LRCLIB tag (optional)</label>
            <input
              type="text"
              id="lrclibTag"
              v-model="lrclibTag"
      ...
      <div class="grow overflow-hidden">
        ...
        <div v-else class="flex flex-col h-full gap-2 overflow-auto">
          <div v-if="searchResult && searchResult.length" class="flex flex-col gap-1 overflow-auto">
            <div v-for="item in searchResult" :key="`${item.provider}-${item.id}`" class="rounded bg-brave-98 dark:bg-brave-10 hover:bg-brave-95 hover:dark:bg-brave-10 border border-transparent hover:dark:border-brave-30 transition px-2 py-1 flex gap-2">
              <div class="h-full overflow-hidden grow">
                <div class="font-bold flex gap-1">
                  ...
                  <span class="text-orange-100 font-bold text-[0.65rem] bg-orange-800 rounded px-1 py-0.5">{{ item.providerName || providerLabels[item.provider] || item.provider }}</span>
                  ...
                  <span v-if="item.duration" class="text-brave-30 text-[0.75rem] bg-brave-95 rounded px-1 py-0.5 dark:text-brave-95 dark:bg-brave-20">
                  ...
                  <span v-if="item.duration && searchingTrack?.duration && Math.round(item.duration) - Math.round(searchingTrack.duration) > 2" class="text-blue-800 text-[0.75rem]">
                    +{{ humanDuration(Math.abs(item.duration - Math.round(searchingTrack.duration))) }}
                  </span>
                  <span v-else-if="item.duration && searchingTrack?.duration && Math.round(item.duration) - Math.round(searchingTrack.duration) < -2" class="text-blue-800 text-[0.75rem]">
                    -{{ humanDuration(Math.abs(item.duration - Math.round(searchingTrack.duration))) }}
                  </span>
                </div>
...
const humanDuration = (seconds) => {
  return new Date((seconds || 0) * 1000).toISOString().slice(14, 19)
}
...
const preview = async (lyricsItem) => {
  try {
    const refreshedLyrics = await invoke('retrieve_search_result_lyrics', { searchResultItem: lyricsItem })
    previewingTrack.value = props.searchingTrack
    previewingLyrics.value = refreshedLyrics
    openPreviewModal()
  } catch (error) {
    console.error(error)
    toast.error(error)
...
const apply = async (lyricsItem) => {
  try {
    const result = await invoke('apply_search_result_lyrics', {
      trackId: props.searchingTrack.id,
      searchResultItem: lyricsItem,
    })

  ...
  >
    <template #titleLeft>
      <div class="flex-none flex gap-1 items-center">
        <VTooltip v-if="lyricsLintResult.length === 0" theme="lyricseekpro-tooltip">
          <Check class="text-lime-500 text-2xl block" />

          <template #popper>
            <div class="text-xs font-bold">No formatting errors detected</div>
          </template>
        </VTooltip>

        <VTooltip v-else-if="plainTextLintResult.length === 0" theme="lyricseekpro-tooltip">
          <AlertCircleOutline class="text-orange-500 text-2xl block" />

          <template #popper>
            <div class="text-xs font-bold">Lyrics are plain text only<br />Add timestamps for synchronized playback</div>
          </template>
        </VTooltip>

        <VTooltip v-else theme="lyricseekpro-tooltip">
          <AlertCircle class="text-red-500 text-2xl block" />

          <template #popper>
            <div class="text-xs font-bold">Lyrics formatting issues detected</div>
          </template>
        </VTooltip>
      </div>
    </template>
...
import { Loading, Minus, Plus, Check, AlertCircleOutline, AlertCircle, MagnifyPlus, MagnifyMinus, Music } from 'mdue'

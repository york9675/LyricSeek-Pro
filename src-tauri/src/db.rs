...
pub fn upgrade_database_if_needed(
    ...
    if existing_version < CURRENT_DB_VERSION {
        ...
        if existing_version <= 7 {
            ...
            tx.execute_batch(indoc! {"
            ALTER TABLE config_data ADD providers_order TEXT DEFAULT 'lrclib,netease,simpmusic,genius';
            "})?;
        ...
        if existing_version <= 8 {
            ...
            tx.execute_batch(indoc! {"
            ALTER TABLE config_data ADD enabled_providers TEXT DEFAULT 'lrclib,netease,simpmusic,genius';
            ALTER TABLE config_data ADD prefer_synced_lyrics BOOLEAN DEFAULT 0;
            "})?;
        ...
        if existing_version <= 9 {
            ...
            tx.execute_batch(indoc! {"
            ALTER TABLE config_data ADD lastfm_links_enabled BOOLEAN DEFAULT 1;
            "})?;
        ...
        if existing_version <= 10 {
            ...
            UPDATE config_data
            SET
              skip_tracks_with_synced_lyrics = 1,
              skip_tracks_with_plain_lyrics = 1,
              show_line_count = 1,
              try_embed_lyrics = 1,
              prefer_synced_lyrics = 1,
              theme_mode = 'light',
              providers_order = 'lrclib,netease,simpmusic,genius',
              enabled_providers = 'lrclib,netease,simpmusic,genius',
              lastfm_links_enabled = 1
            WHERE 1;
            "})?;
...
pub fn get_config(db: &Connection) -> Result<PersistentConfig> {
    ...
    let row = statement.query_row([], |r| {
        let providers_order_raw: Option<String> = r.get("providers_order")?;
        let enabled_providers_raw: Option<String> = r.get("enabled_providers")?;
        Ok(PersistentConfig {
            ...
            providers_order: providers_order_raw
                .map(|raw| providers::parse_providers_order(&raw))
                .unwrap_or_else(providers::default_providers_order),
            enabled_providers: enabled_providers_raw
...
pub fn set_config(
    ...
    let enabled_providers = providers::stringify_enabled_providers(enabled_providers);
...
pub fn add_album(name: &str, album_artist_name: &str, db: &Connection) -> Result<i64> {
    add_album_with_cover(name, album_artist_name, None, db)
}

pub fn add_album_with_cover(name: &str, album_artist_name: &str, cover_art_path: Option<String>, db: &Connection) -> Result<i64> {
    let mut statement = db.prepare("INSERT INTO albums (name, name_lower, album_artist_name, album_artist_name_lower, image_path) VALUES (?, ?, ?, ?, ?)")?;
...
pub fn add_track(track: &fs_track::FsTrack, db: &Connection) -> Result<()> {
    ...
    let album_id = match album_result {
        ...
        Err(_) => add_album_with_cover(&track.album(), &track.album_artist(), track.cover_art_path(), db)?,
    };

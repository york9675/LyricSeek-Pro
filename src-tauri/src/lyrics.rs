...
pub enum GetLyricsError {
    #[error("This track does not exist in configured lyric providers")]
    NotFound,
}

pub struct DownloadLyricsResult {
    pub lyrics: Response,
    pub provider: String,
}
...
) -> Result<DownloadLyricsResult> {
    let active_providers = providers::ordered_active_providers(providers_order, enabled_providers);
    let mut fallback_plain: Option<(Response, String)> = None;

    for provider in active_providers {
        match providers::request_by_provider(&provider, &track, lrclib_instance).await {
            Ok(Response::None) => {
                continue;
            }
            Ok(Response::UnsyncedLyrics(plain_lyrics)) => {
                if prefer_synced_lyrics {
                ...
                let lyrics = apply_lyrics_for_track(
                    track,
                    Response::UnsyncedLyrics(plain_lyrics),
                    is_try_embed_lyrics,
                )
            ...
            Err(error) => {
                eprintln!(
                    "Failed to fetch lyrics from provider {}: {}",
                    provider,
                    error
                );
            }
